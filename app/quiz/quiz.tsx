...
import { Answers, Question } from "~/quiz/quiz.type";
import Button from "~/shared/components/button";
import { ArrowLeftIcon } from "@radix-ui/react-icons";
import { Progress } from "~/shared/components/progress";
import OptionsHandler from "~/quiz/components/options-handler";
...
export default function Quiz() {
  ...
  const handleSubmit = (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    if (questionId === lastQuestionId) {
      ...
      const prevAnswers = lo.omit(initialState, "questions");
      //Initialize data with previous and recently submitted data
      const newAnswers = lo.merge(prevAnswers || {}, data);
      // Log form submission
      ...
      const newState = lo.merge(initialState || {}, data);
      alert(JSON.stringify(lo.omit(newState, "questions")));
        ...
        <fetcher.Form method="post" onSubmit={handleSubmit}>
          ...
          <div className="flex fixed z-20 bottom-8 right-0 left-0">
            <Button
              ...
              type="submit"
              disabled={isSubmitting || questionIndex === questionKeys.length}
            >
              {isSubmitting
                ? "Submitting"

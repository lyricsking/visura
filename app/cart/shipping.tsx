...
import { useFetcher, useLoaderData, json } from 'remix';
...
export let loader = async ({ request }) => {
  ...
  const addresses = [
    { type: 'Home', address: '3501 Maloy Court, East Elmhurst, New York City, NY 11369', phone: '78596 0000', selected: true },
    { type: 'Office', address: '8502-8503 Preston Rd. Inglewood Street, Maine 98380', phone: '12100 0023', selected: false },
  ];
...
const ShippingDetails: React.FC = () => {
  const { addresses, showForm } = useLoaderData();
  ...
  const toggleFormVisibility = () => {
    if (showForm) {
      ...
      fetcher.submit({SHOW_ADDRESS_FORM: true})
    }
  };
      ...
      {showForm && (
        <fetcher.Form method="post" className="border p-4 rounded-lg mb-4">
          ...
          <div className="mb-2">
            ...
            <input name="address" type="text" className="w-full border border-gray-300 rounded-lg p-2" required />
          </div>
          <div className="mb-2">
            <label className="block text-gray-700">Phone</label>
            <input name="phone" type="text" className="w-full border border-gray-300 rounded-lg p-2" required />
          </div>

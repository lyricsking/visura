  ...
  return (
    <div className={className}>
      {socials.map((social) => {
        switch (social) {
          case "facebook":
            return <Facebook key={social} {...otherProps} />;
          case "whatsapp":
            return <WhatsApp key={social} {...otherProps} />;
        }
      })}
    </div>
  );
...
export type Socials = keyof typeof Socials;

function Facebook(props: Omit<Props, "socials">) {
  const color = "#1877f2";
  ...
  return (
    <a href="#!" className={props.wrapperClass} style={style}>
      <svg
...
function WhatsApp(props: Omit<Props, "socials">) {
  const color = "#128c7e";
  const style: React.CSSProperties = {
    ...(props.iconOnly
      ? { color }
